    ...
    protected SecurityFilterChain configure(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(
                request -> request
                    .requestMatchers("/rest/server*").authenticated()
            )
//            .formLogin(Customizer.withDefaults())
    ...
    public UserDetailsManager userDetailsService() {
        ...
        manager.createUser(User.withUsername("kieserver")
            .password(encoder.encode(KIE_SERVER_PASSWORD))
            .roles(KIE_SERVER_ROLE)
            .build());
        manager.createUser(User.withUsername("yoda")
            .password(encoder.encode(USER_PASSWORD))
            .roles(KIE_SERVER_ROLE, GUEST_ROLE)
            .build());
        manager.createUser(User.withUsername("administrator")
            .password(encoder.encode(USER_PASSWORD))
            .roles(KIE_SERVER_ROLE, GUEST_ROLE, ADMIN_ROLE)
            .build());
        manager.createUser(User.withUsername("john")
            .password(encoder.encode(USER_PASSWORD))
            .roles(KIE_SERVER_ROLE, GUEST_ROLE, ENGINEERING_ROLE, HR_ROLE, IT_ROLE, ACCOUNTING_ROLE)
            .build());
